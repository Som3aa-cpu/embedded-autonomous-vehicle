...
static uint8_t PercentToDuty(uint8_t speedPercent)
{
    if (speedPercent > 100u)
    {
        speedPercent = 100u;
    }
    return (uint8_t)(((uint16_t)speedPercent * MOTOR_PWM_MAX_DUTY) / 100u);
}
...
void Motor_Init(void)
{
    ...
    /* Configures Timer0 (OC0) and Timer1 (OC1A) in Fast PWM, non-inverting */
    FastPWNinit();

    Timer0CompareMatchValue(0);
